...
parser.add_option("-s", "--sample", dest="sample", default="MisIDEle",type='str',
...
#----------------------------------------
inHistSubDir = "Hist_Ntuple/HistMisIDSF/forMisIDSF/%s/%s/%s/Merged"%(year, decayMode, channel)
inHistFullDir = "%s/%s"%(condorHistDir, inHistSubDir)
outPlotSubDir = "Plot_Hist/PlotMisIDSF/forMisIDSF/%s/%s/%s/%s"%(year, decayMode, channel, region)
...
for index, syst in enumerate(Systematics):
    hPathBase = "%s/%s/%s/Base"%(sample, region, hName)
    hPathUp   = "%s/%s/%s/%sUp"%(sample, region, hName, syst)
    hPathDown   = "%s/%s/%s/%sDown"%(sample, region,hName,syst)
    print hPathBase
    print hPathUp
