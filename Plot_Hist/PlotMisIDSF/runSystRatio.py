...
texFile = open("tex/systRatio.tex", "w")
...
for plot in allHistList:
    ...
    for d, c in itertools.product(Decay, Channel):
        for r in Regions.keys():
            for s in SampleBkg.keys():
                for y in Year_:
                    args1 = "-y %s -d %s -c %s --hist %s -r %s -s %s"%(y, d, c, plot, r, s)
                    ...
                    if "tty_Enriched" in r and "Reco_mass_T" in plot:
                        continue
                    plotDir  = "%s/Plot_Hist/PlotMisIDSF/forMisIDSF/%s/%s/%s/%s"%(condorHistDir, y, d, c, r)
