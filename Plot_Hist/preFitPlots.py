...
def makePlot(hName, CR, isQCDMC, isData, isLog, isRatio, isUnc):
    ...
    sigFile = TFile("%s/%s.root"%(inHistFullDir,"TT_tgtg_M800"), "read")
    sigHist = sigFile.Get('%s/Base/SR/%s'%('TT_tgtg_M800', hName))
    sigHist.Scale(18)
    ...
    plotLegend.AddEntry(sigHist, "18 x signal tgtg (mT = 800 GeV)", "PL")
    plotLegend.Draw()
