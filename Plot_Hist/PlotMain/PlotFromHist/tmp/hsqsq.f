     ...
     .            GLPM,GHPM,B,A
      ...
      COMMON/TRILINEAR_HDEC/AU,AD,AU1,AD1
...
1     II=II+1
     ...
     .                  AMST(2),STHT,CTHT,GLTT(2,2),GHTT(2,2),GATT
      ...
      COMMON/TRILINEAR_HDEC/AT1,AB1,AT,AB
      COMMON/BREAKSCALE_HDEC/SUSYSCALE
      COMMON/SQUARKHIGGS_HDEC/THEB,AMG,IONSH,IDTH
      COMMON/HHSSNLO_HDEC/FLLTT(2,2),FHHTT(2,2),FLHTT(2,2)
      IF(ITHETA.NE.0)THEN
       AT = AT1
       AB = AB1
      ENDIF
...
C   ********************************************************************
      ...
      COMMON/FEYNHIGGS0_HDEC/IFEYNHIGGS,ITHETA
      IF(ITHETA.EQ.0) THEN
c--with shift in A_b/t
       CALL SFERMHO0_HDEC(AMSQ,AMUR,AMDR,AU,AD,AMU,TB,AMW,AMZ)
      ELSE
c--without shift in A_b/t
       CALL SFERMHO1_HDEC(AMSQ,AMUR,AMDR,AU,AD,AMU,TB,AMW,AMZ)
      ENDIF
      RETURN
      END 

      SUBROUTINE SFERMHO0_HDEC(AMSQ,AMUR,AMDR,AU,AD,AMU,TB,AMW,AMZ)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON/MASSES_HDEC/AMS,AMC,AMB,AMT
      COMMON/SQNLO_HDEC/AMSB(2),STHB,CTHB,GLBB(2,2),GHBB(2,2),GABB,
     .                  AMST(2),STHT,CTHT,GLTT(2,2),GHTT(2,2),GATT
      COMMON/SQUARKHIGGS_HDEC/THEB,AMG,IONSH,IDTH
      COMMON/BREAKSCALE_HDEC/SUSYSCALE
      COMMON/TRILINEAR_HDEC/AU0,AD0,AU1,AD1
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
C
      ...
      ENDIF
      FAC = 1
      CALL SFERMIT00_HDEC(ISO,QQ,RMT,AMG,AMSQ,AMUR,AU,AMU,TB,
     .             AMW,AMZ,AMST(1),AMST(2),RMSQ,RMUR,RAU,THET,FAC)
      AU1 = RAU
      CTHT= DCOS(THET)
      STHT= DSIN(THET) 
      ...
      AMSQR2 = RMUR**2 + DR
      AMLR   = RAU - AMU/TB
      DEL    = (AMSQL2-AMSQR2)**2+4*RMT**2*AMLR**2
      AMSQ12 = RMT**2 + (AMSQL2+AMSQR2-DSQRT(DEL))/2
      AMSQ22 = RMT**2 + (AMSQL2+AMSQR2+DSQRT(DEL))/2
      AM1=DSQRT(AMSQ12)
      AM2=DSQRT(AMSQ22)
      IF(AMSQL2.EQ.AMSQR2) THEN
       THET = PI/4
      ELSE
       THET=0.5D0*DATAN(2.D0*RMT*AMLR / (AMSQL2-AMSQR2) )
       IF(AMSQL2.GT.AMSQR2) THET = THET + PI/2
      ENDIF
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c     write(6,*)'stops:'
c     write(6,*)'======'
      XXL = DSQRT(RMSQ**2-AMT**2+RMT**2)
      XXR = DSQRT(RMUR**2-AMT**2+RMT**2)
      RMSQ0 = XXL
      RMUR0 = XXR
      STHT0 = STHT
      CTHT0 = CTHT
...
C
      ...
      ENDIF
      FAC = 1
      CALL SFERMIT01_HDEC(ISO,QQ,RMB,AMG,AMSQ,AMDR,AD,AMU,TB,
     .             AMW,AMZ,AMSB(1),AMSB(2),RMSQ,RMDR,RAD,THEB)
      AD1 = RAD
      CTHB= DCOS(THEB)
      STHB= DSIN(THEB)
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c     write(6,*)'sbottoms:'
c     write(6,*)'========='
      XXL = DSQRT(RMSQ**2-AMB**2+RMB**2)
      XXR = DSQRT(RMDR**2-AMB**2+RMB**2)
      RMDR0 = XXR
      STHB0 = STHB
      CTHB0 = CTHB
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      SUBROUTINE SFERMHO1_HDEC(AMSQ,AMUR,AMDR,AU,AD,AMU,TB,AMW,AMZ)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON/MASSES_HDEC/AMS,AMC,AMB,AMT
      COMMON/SQNLO_HDEC/AMSB(2),STHB,CTHB,GLBB(2,2),GHBB(2,2),GABB,
     .                  AMST(2),STHT,CTHT,GLTT(2,2),GHTT(2,2),GATT
...
c     ENDIF
      CTHT= DCOS(THET)
      STHT= DSIN(THET) 
c     write(1,*)'stht: ',THET,STHT,CTHT
c     write(6,*)'sfermho'
c     write(6,*)'theta = ',thet
...
c     write(6,*)'st   = ',stht
c     write(6,*)'ct   = ',ctht
c     write(6,*)'s2t  = ',2*stht*ctht
c     write(6,*)'s2t  = ',dsin(2*thet)
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c     write(6,*)'stops:'
c     write(6,*)'======'
      XXL = DSQRT(RMSQ**2-AMT**2+RMT**2)
      XXR = DSQRT(RMUR**2-AMT**2+RMT**2)
c     write(6,*)'NLO: M_L = ',XXL,' M_R = ',XXR
c     write(6,*)'LO:  M_L = ',AMSQ,' M_R = ',AMUR
c     write(6,*)
      RMSQ0 = XXL
      RMUR0 = XXR
c     STHT= DSIN(THET)
...
c     write(6,*)'========='
      ...
      XXR = DSQRT(RMDR**2-AMB**2+RMB**2)
c     write(6,*)'NLO: M_L = ',XXL,' M_R = ',XXR
c     write(6,*)'LO:  M_L = ',AMSQ,' M_R = ',AMDR
c     write(6,*)
      RMDR0 = XXR
      STHB0 = STHB
      CTHB0 = CTHB
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      SUBROUTINE SFERMIT00_HDEC(ISO,QQ,RMQ0,AMG,AML,AMR,AQ,AMU,TB,
     .                        AMW,AMZ,AM1,AM2,RML,RMR,RAQ,THET,FACT)
c--with shift in A_t
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
100   RML = RML0
     ...
     .     + (2*RMQ**2+(AM2**2-AM1**2)*S2T**2/2)*DMQ
      ...
      IF(IDTH.NE.0)THEN
       ...
       C2T = CT**2-ST**2
       S2T = 2*ST*CT
       A1 = RM1**2*(1+DLOG(XMU**2/RM1**2))
       A2 = RM2**2*(1+DLOG(XMU**2/RM2**2))
       AQQ= RMQ**2*(1+DLOG(XMU**2/RMQ**2))
       AG = AMG**2*(1+DLOG(XMU**2/AMG**2))
       B101 = B02_HDEC(AM1**2,0.D0,RM1,XMU**2)
       B202 = B02_HDEC(AM2**2,0.D0,RM2,XMU**2)
       B1GQ = B02_HDEC(AM1**2,AMG,RMQ,XMU**2)
       B2GQ = B02_HDEC(AM2**2,AMG,RMQ,XMU**2)
       BQG1 = B02_HDEC(RMQ**2,AMG,RM1,XMU**2)
       BQG2 = B02_HDEC(RMQ**2,AMG,RM2,XMU**2)
       B1QG1 = BB1(RMQ**2,AMG,RM1,XMU)
       B1QG2 = BB1(RMQ**2,AMG,RM2,XMU)
       DMQ = -(3*DLOG(XMU**2/QQ**2)+B1QG1+B1QG2
     .       + AMG*2*(AQ-AMU*RQ)/(RM1**2-RM2**2)*(BQG1-BQG2)
     .       + DELSUSY) + DELTA_Q
       DM12 = (1+C2T**2)*A1+S2T**2*A2-2*AG-2*AQQ-4*RM1**2*B101
     .      - 2*(AMG**2+RMQ**2-RM1**2-2*AMG*RMQ*S2T)*B1GQ
       DM22 = (1+C2T**2)*A2+S2T**2*A1-2*AG-2*AQQ-4*RM2**2*B202
     .      - 2*(AMG**2+RMQ**2-RM2**2+2*AMG*RMQ*S2T)*B2GQ
     ...
     .         +B02_HDEC(AM2**2,AMG,RMQ,XMU**2))/2
       ...
       ENDIF
       DTH = (S2T*C2T*(A2-A1)+4*AMG*RMQ*C2T*BTGQ)/(RM1**2-RM2**2)
       DTH0= S2T/2/C2T*(DMQ + DAQ/(AQQ-AMU*RQ)
     .     - (DM12-DM22)/(RM1**2-RM2**2))
       DTH = ALP*DTH
       DTH0= ALP*DTH0
       THET = THET0 + DTH0 - DTH
       DTH = THET-THET0
       RAQ = DSIN(2*THET)*(RM1**2-RM2**2)/2/RMQ + AMU*RQ
      ENDIF
      CT= DCOS(THET)
      ST= DSIN(THET) 
      C2T = CT**2-ST**2
     ...
     .     - (AMSQL20+AMSQR20+DSQRT(DEL0))/2
      ...
      XMSQL2 = RML0**2 + DL
      XMSQR2 = RMR0**2 + DR
      DEL    = (XMSQL2-XMSQR2)**2+4*RMQ**2*RMLR**2
      ...
      IF(XMSQL2.EQ.XMSQR2) THEN
       THET0 = PI/4
      ELSE
       THET0=0.5D0*DATAN(2.D0*RMQ*RMLR / (XMSQL2-XMSQR2) )
      ...
      IF(XMSQL2.EQ.XMSQR2) THEN
       THET = PI/4
      ELSE
       THET=0.5D0*DATAN(2.D0*RMQ*RMLR / (XMSQL2-XMSQR2) )
      ...
      SUBROUTINE SFERMIT01_HDEC(ISO,QQ,RMQ0,AMG,AML,AMR,AQ,AMU,TB,
     .                        AMW,AMZ,AM1,AM2,RML,RMR,RAQ,THET)
c--with shift in A_b
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON/SQUARKHIGGS_HDEC/THEB,XMG,IONSH,IDTH
      BB1(P2,AM1,AM2,XMU)=((AM1**2-AM2**2)*B02_HDEC(0.D0,AM1,AM2,XMU**2)
...
c100   III = III+1
     ...
     .     + (2*RMQ**2+(AM2**2-AM1**2)*S2T**2/2)*DMQ
      ...
      IF(IDTH.NE.0)THEN
       ...
       AQQ= RMQ**2*(1+DLOG(XMU**2/RMQ**2))
       AG = AMG**2*(1+DLOG(XMU**2/AMG**2))
       B101 = B02_HDEC(RM1**2,0.D0,RM1,XMU**2)
       B202 = B02_HDEC(RM2**2,0.D0,RM2,XMU**2)
       B1GQ = B02_HDEC(RM1**2,AMG,RMQ,XMU**2)
       B2GQ = B02_HDEC(RM2**2,AMG,RMQ,XMU**2)
       BQG1 = B02_HDEC(RMQ**2,AMG,RM1,XMU**2)
       BQG2 = B02_HDEC(RMQ**2,AMG,RM2,XMU**2)
       B1QG1 = BB1(RMQ**2,AMG,RM1,XMU)
       B1QG2 = BB1(RMQ**2,AMG,RM2,XMU)
       IF(IDTH.EQ.1)THEN
        BTGQ = (B02_HDEC(RM1**2,AMG,RMQ,XMU**2)
     .         +B02_HDEC(RM2**2,AMG,RMQ,XMU**2))/2
       ELSEIF(IDTH.EQ.21)THEN
        BTGQ = B02_HDEC(RM1**2,AMG,RMQ,XMU**2)
       ELSEIF(IDTH.EQ.22)THEN
        BTGQ = B02_HDEC(RM2**2,AMG,RMQ,XMU**2)
       ENDIF
       DMQ = -(3*DLOG(XMU**2/QQ**2)+B1QG1+B1QG2
     .       + AMG*2*(AQ-AMU*RQ)/(RM1**2-RM2**2)*(BQG1-BQG2)
     .       + DELSUSY) + DELTA_Q
     ...
     .     - (DM12-DM22)/(RM1**2-RM2**2))
       ...
       DTH = THET-THET0
c--shift A_Q
       QS = (RM1+RM2+DABS(AMG))/3
       ALPS = CF*ALPHAS_HDEC(QS,3)/4/PI
       DELTA_Q = 2*AMG*AMU*RQ*T_HDEC(RM1,RM2,AMG)
       RMQ = RMQ0/(1+ALPS*DELTA_Q)
       RAQ = DSIN(2*THET)*(RM1**2-RM2**2)/2/RMQ + AMU*RQ
      ENDIF
      CT= DCOS(THET)
      ...
      AMSQL2 = RML0**2 + DL
      AMSQR2 = RMR0**2 + DR
      DEL    = (AMSQL2-AMSQR2)**2+4*RMQ**2*RMLR**2
      ...
      IF(AMSQL2.EQ.AMSQR2) THEN
       THET = PI/4
      ELSE
       THET=0.5D0*DATAN(2.D0*RMQ*RMLR / (AMSQL2-AMSQR2) )
       IF(AMSQL2.GT.AMSQR2) THET = THET + PI/2
      ENDIF
      enddo
c--end iteration
...
c     DELTA_Q = 2*AMG*AMU*RQ*T_HDEC(AM1,AM2,AMG)
      RMQ1 = RMQ0/(1+ALPS*DELTA_Q)
      FACT = RMQ1/RMQ0
c     write(6,*)'iteration',idth
c     write(6,*)'st   = ',dsin(thet)
c     write(6,*)'ct   = ',dcos(thet)
c     write(6,*)'s2t  = ',2.D0*RMQ*AMLR / (XMSQ12-XMSQ22)
c     write(6,*)'s2tp = ',2*dsin(thet)*dcos(thet)
c     write(6,*)'s2tp = ',dsin(2*thet)
c     write(6,*)'sin(th)  = ',dsin(thet),'cos(th)  = ',dcos(thet)
c     write(6,*)'sin(th0) = ',dsin(thet0),'cos(th0) = ',dcos(thet0)
      RETURN
      END 
...
c     write(6,*)'Check:     ',III,AM1,AM2,RML0,RMR0
c     write(6,*)'Iteration: ',III,2.D0*RMQ*AMLR / (AMSQL2-AMSQR2)
      enddo
c--end iteration
      RML = RML0
      RMR = RMR0
