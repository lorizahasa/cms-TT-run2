...
#----------------------------------------
inHistSubDir = "/Hist_Ntuple/HistMain/Rebin/%s/%s/%s/Merged"%(year, decayMode, channel)
inHistFullDir = "%s/%s"%(condorHistDir, inHistSubDir)
inFile = TFile("%s/AllInc.root"%(inHistFullDir), "read")
outPlotSubDir = "Plot_Hist/PlotMain/Rebin/%s/%s/%s/%s"%(year, decayMode, channel, region)
