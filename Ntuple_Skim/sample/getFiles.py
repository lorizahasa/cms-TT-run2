...
for year in Years: 
    ...
    for sampleName, fEvt in sampleList.items():
        ...
        fileList = subprocess.Popen('eos root://cmseos.fnal.gov/ ls %s/%s/%s'%(outSkimDir, year, extraArgs),shell=True,stdout=subprocess.PIPE).communicate()[0].decode("ascii").split('\n')
        fileList.remove("")
        nFiles = len(fileList)
