...
for err in errSearch:
    grepList = subprocess.Popen('grep -rn %s %s'%(err, logDir),shell=True,stdout=subprocess.PIPE).communicate()[0].split('\n')
    grepList.remove("")
    grepLists += grepList
errList = []
...
for year, decay, ch in itertools.product(Years, Decays, Channels):
    ...
    #----------------------------------------
    outDir="%s/Read/Reader/%s/%s/%s/CombMass/BDTA"%(dirRead, year, decay, ch)
