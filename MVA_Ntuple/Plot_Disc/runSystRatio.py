...
texFile = open("tex/systRatio%s.tex"%ext, "w")
...
Mass = ["CombMass"]
samples = ["AllBkgs"]
for s, r, d, c, y in itertools.product(samples, regionList, Decay, Channel, Year_):
    for m in Mass:
        for h in discDict.keys(): 
            for method in discDict[h]:
                ...
                args1 = "-y %s -d %s -c %s --hist %s -r %s --allBkgs "%(y, d, c, h, r)
                ...
                plotPath = "%s/SystRatio_%s_%s_%s.pdf"%(plotDir, h, r, s)
