...
pathMisIDSF = "/eos/uscms/store/user/rverma/Output/cms-TT-run2/CBA_Ntuple/Fit_Hist/FitMisIDSF"
sys.path.insert(0, pathDYSF) 
sys.path.insert(0, pathMisIDSF) 
from FitDYSF_dictDYSF import DYSF
from FitMisIDSF_dictMisIDSF import MisIDSF
...
for decay, region, hName, channel, year in itertools.product(Decays, rList, hList, Channels, Years):
    sfDYKey = "DYSF_%s_%s_%s_%s"%(year, "Dilep", "Mu__Ele", "DY_Enriched_a2j_e0b_e0y")
    sfMisIDKey = "MisIDSF_%s_%s_%s_%s"%(year, "Semilep", channel, region)
    sfDY  = DYSF[sfDYKey]
    sfMisIDs  = MisIDSF[sfMisIDKey]
    sfMisID  = sfMisIDs[0]
    sfZGamma = sfMisIDs[1]
    sfWGamma = sfMisIDs[2]
    print(sfDYKey, sfDY)
    print(sfMisIDKey, sfMisIDs)
    #-----------------------------------------
    #Basic flags
    #----------------------------------------
    isData   = True
    ...
    def makePlot(hName, region, isSig, isData, isLog, isRatio, isUnc):
        ...
        crName = "#splitline{(%s), #color[4]{DYSF = %s}}{#color[4]{%s=%s, ZGSF=%s, WGSF=%s}}"%(crName, str(round(sfDY,2)), dir_, str(round(sfMisID, 2)), str(round(sfZGamma, 2)), str(round(sfWGamma, 2)))
