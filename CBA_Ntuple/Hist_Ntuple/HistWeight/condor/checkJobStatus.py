...
for year, decay, ch in itertools.product(Years, Decays, Channels):
    ...
    for s, r in itertools.product(Samples, Regions.keys()):
        for corr in Corrs.keys():
            for c in Corrs[corr]:
                if "Ele" in ch and "mu" in c: continue
                if "Mu" in ch and "ele" in c: continue
                if "Uncorr" in c and "Uncorr" not in corr: continue
                if "data_obs" in s and "Uncorr" not in corr: continue
                rootFile = "%s_%s_%s.root"%(s, r, c)
                rootFile2 = "%s__%s__%s.root"%(s, r, c)
    ...
    if len(unFinishedList) ==0 and len(corruptedList)==0:
        ...
        for f in totResubJobs :
            ...
            localFile.write('python makeHists.py -y %s -d %s -c %s -r %s -s %s --syst %s \n' %(year, decay, ch, reg, samp, syst))
    print(outDir)
jdlFile.close() 
localFile.close()
