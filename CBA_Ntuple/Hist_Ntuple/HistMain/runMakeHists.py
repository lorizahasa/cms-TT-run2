...
for r in Regions.keys():
    if "tty_" in r:
        args = "-y %s -d %s -c %s -s %s -r %s --syst %s --allHists --isCat"%(year, decay, channel, sample, r, syst)
    else:
        args = "-y %s -d %s -c %s -s %s -r %s --syst %s --allHists"%(year, decay, channel, sample, r, syst)
