...
#----------------------------------------
inFileName = "%s/Hist_Ntuple/HistDYSF/forDYSF/%s/%s/%s/Merged/AllInc.root"%(condorHistDir, year, decayMode, channel)
print inFileName
inHistDirBase   = "$PROCESS/%s/$BIN/Base"%region
inHistDirSys    = "$PROCESS/%s/$BIN/$SYSTEMATIC"%region
outFileDir      = "%s/Fit_Hist/FitDYSF/forDYSF/%s/%s/%s/%s/%s"%(condorHistDir, year, decayMode, channel, region, hName)
