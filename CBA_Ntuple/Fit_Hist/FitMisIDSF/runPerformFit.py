...
if isComb:
    for y, d, r in itertools.product(Year, Decay, Regions): 
        ...
        print args
        plotDir  = "%s/Fit_Hist/FitMisIDSF/forMisIDSF/%s/%s/%s/%s/%s"%(condorHistDir, y, d, "Mu_Ele", r, hName)
        if isFD:
            runCmd("python performFit.py %s --isT2W --isFD"%args)
        if isImpact:
            runCmd("python performFit.py %s --isT2W --isImpact"%args)
    ...
    for d, c, r in itertools.product(Decay, Channel, Regions): 
        ...
        print args
        plotDir  = "%s/Fit_Hist/FitMisIDSF/forMisIDSF/%s/%s/%s/%s/%s"%(condorHistDir, "2016_2017_2018", d, c, r, hName)
        if isFD:
            runCmd("python performFit.py %s --isT2W --isFD"%args)
        if isImpact:
            runCmd("python performFit.py %s --isT2W --isImpact"%args)
    ...
    for d, r in itertools.product(Decay, Regions): 
        ...
        print args
        plotDir  = "%s/Fit_Hist/FitMisIDSF/forMisIDSF/%s/%s/%s/%s/%s"%(condorHistDir, "2016_2017_2018", d, "Mu_Ele", r, hName)
        if isFD:
            runCmd("python performFit.py %s --isT2W --isFD"%args)
        if isImpact:
            runCmd("python performFit.py %s --isT2W --isImpact"%args)
